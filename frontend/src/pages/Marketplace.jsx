...
import { fetchListings, setFilters } from '../store/slices/marketplaceSlice';
...
const Marketplace = () => {
  ...
  const { items, filters, status } = useSelector((state) => state.marketplace);
  ...
  // getCropImage imported from utils/cropImages.js

  const handleSearchSubmit = (e) => {
    e.preventDefault();
  };

  // Client-side filtering — works instantly without hitting the backend
      ...
      >
        <div className="flex justify-between items-center mb-6 text-white">
          ...
          >
             ...
             <p className="text-[10px] uppercase font-bold tracking-widest opacity-80">PS-302 Hackathon Optimized</p>
          </motion.div>
        ...
        >
          <input 
            ...
            className="w-full h-14 bg-white rounded-2xl pl-14 pr-4 text-gray-900 font-bold outline-none focus:ring-4 ring-white/20 transition-all shadow-lg text-sm"
            value={localSearch}
            onChange={(e) => { setLocalSearch(e.target.value); setActiveChip('all'); }}
          />
          <Search className="absolute left-5 top-4.5 w-5 h-5 text-gray-400" />
        </motion.div>
      </motion.div>
             ...
             <motion.button 
               ...
               className={`px-5 py-2.5 border rounded-full text-xs font-black whitespace-nowrap shadow-sm transition-all active:scale-95 ${
                 activeChip === crop
                   ...
                   : 'bg-white text-gray-500 border-gray-100 hover:bg-primary hover:text-white'
               }`}
             >
              ...
              {filteredItems.map((listing, i) => {
                ...
                return (
                  <motion.div 
                    ...
                    key={listing._id} 
                    className="group bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-2xl transition-all duration-500 hover:-translate-y-1"
                  >
                    ...
                    <div className="p-4">
                       <h3 className="text-base font-black text-gray-900 mb-1">{listing.cropName}</h3>
                       <div className="flex items-center gap-1 mb-3">
                          <Star className="w-3 h-3 text-accent fill-accent" />
                          <span className="text-[9px] font-bold text-gray-400">4.9 (12)</span>
                       </div>

                       <div className="flex items-baseline justify-between mb-3">
                          <p className="text-xl font-black text-primary">₹{listing.price}</p>
                          <p className="text-[9px] font-bold text-gray-400 uppercase">/Qtl</p>
                       </div>

                       {/* Mandi benchmark compact */}
                       <div className="bg-gray-50 p-2.5 rounded-xl flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                             <History className="w-3.5 h-3.5 text-primary" />
                             <span className="text-[9px] font-bold text-gray-500">Mandi ₹{listing.mandiPrice || '---'}</span>
                          </div>
                          <div className={`px-2 py-0.5 rounded-lg text-[8px] font-black ${isFairPrice ? 'bg-green-100 text-[#2E7D32]' : 'bg-red-100 text-red-600'}`}>
                       ...
                       <button 
                         ...
                         className="w-full h-11 bg-gray-900 text-white font-black text-xs rounded-xl flex items-center justify-center gap-2 hover:bg-primary transition-all shadow-lg active:scale-95"
                       >
                         Confirm Purchase
                       </button>
                    </div>
