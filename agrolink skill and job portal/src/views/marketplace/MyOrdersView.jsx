...
const MyOrdersView = () => {
  ...
  useEffect(() => {
    ...
    const q = query(
      ...
      or(
        where('buyerId', '==', user._id || 'demo_buyer'),
        where('farmerId', '==', user._id || 'demo_farmer')
      )
    );
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      default: return <Clock className="w-5 h-5 text-charcoal/40" />;
    }
  };
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      default: return 'bg-light-gray text-charcoal/60 border-light-gray';
    }
  };
       ...
       <div className="p-6 space-y-4">
          {orders.length === 0 ? (
            <div className="py-20 text-center">
               ...
               <p className="text-charcoal/40 font-bold">No orders found yet.</p>
               <button 
            ...
            <AnimatePresence>
               {orders.map((order, i) => (
                 <motion.div 
                   ...
                   className="bg-white rounded-[32px] p-5 shadow-sm border border-light-gray flex items-center gap-4 hover:shadow-md transition-all active:scale-[0.98] cursor-pointer"
                 >
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border ${getStatusColor(order.orderStatus)}`}>
                       {getStatusIcon(order.orderStatus)}
                    </div>
                    <div className="flex-1">
                       <h3 className="font-black text-charcoal leading-tight">Order for {order.quantity} Qtl Crop</h3>
                       <div className="flex items-center gap-2 mt-1">
                          <span className="text-[10px] font-bold text-charcoal/40 uppercase tracking-wider">₹{order.totalPrice}</span>
                          <span className="w-1 h-1 bg-light-gray rounded-full" />
                          <span className="text-[10px] font-bold text-charcoal/40 uppercase tracking-wider">{new Date(order.createdAt).toLocaleDateString()}</span>
                       </div>
