...
const ListingDetail = () => {
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#FAFAF7]">
       ...
       <p className="font-black text-gray-400 animate-pulse">{t('loading_data')}</p>
    </div>
  );
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="min-h-screen bg-[#FAFAF7] font-sans pb-32"
    >
      <div className="relative h-96 w-full bg-gray-200 overflow-hidden">
        <motion.button 
          ...
          whileTap={{ scale: 0.9 }}
          onClick={() => navigate(-1)} 
          className="absolute top-6 left-6 z-30 p-3 bg-white/90 backdrop-blur rounded-2xl shadow-lg border border-gray-100"
        >
      ...
      >
        <div className="bg-white rounded-[40px] p-8 shadow-2xl shadow-gray-200/50 border border-gray-100">
          <div className="flex justify-between items-start mb-6">
            ...
            >
              ...
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">{t('per_quintal')}</p>
            </motion.div>
          </div>

          <div className="grid grid-cols-2 gap-4 mb-8">
             <motion.div whileHover={{ y: -5 }} className="bg-gray-50 p-5 rounded-3xl border border-gray-100">
                <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">{t('weight_label')}</p>
                <p className="text-xl font-black">{listing.quantity} Quintals</p>
             </motion.div>
             <motion.div whileHover={{ y: -5 }} className="bg-gray-50 p-5 rounded-3xl border border-gray-100">
                <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Mandi Hub</p>
          ...
          >
             ...
             <div className="flex-1">
                <p className="font-extrabold text-gray-900">{listing.farmerId?.name || 'Ramesh Singh'}</p>
                <div className="flex items-center gap-1">
                   {[1,2,3,4,5].map(s => <Star key={s} className="w-3 h-3 text-accent fill-accent" />)}
                   <span className="text-xs text-gray-400 font-bold ml-1">4.9 (12 reviews)</span>
                </div>
             </div>
             <motion.button 
          ...
          <section className="mb-8">
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <p className="text-sm font-bold text-gray-800 leading-snug">"Superb quality rice. Delivery was timely. The moisture levels were perfect as described."</p>
                <p className="text-[10px] font-bold text-gray-400 mt-2 uppercase">— Arun Kumar, 2 days ago</p>
              </motion.div>
            </div>
          </section>
          ...
          <motion.div 
            whileHover={{ scale: 1.02 }}
            className="bg-gray-900 text-white p-7 rounded-[32px] overflow-hidden relative mb-8 shadow-2xl"
          >
